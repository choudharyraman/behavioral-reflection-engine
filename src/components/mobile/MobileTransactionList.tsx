...
export function MobileTransactionList({ transactions, onAddTag, onEmotionTag }: MobileTransactionListProps) {
  ...
  return (
    <div className="flex flex-col h-full pb-24">
      {/* M3 Search and Filter Header */}
      <div className="sticky top-0 z-30 bg-background/80 backdrop-blur-xl border-b border-border/50 px-5 py-4 space-y-4">
        <div className="relative max-w-2xl mx-auto">
          ...
          <Input
            ...
            className="pl-12 h-12 rounded-full bg-secondary border-0 shadow-sm text-base font-medium"
          />
          {searchQuery && (
            <Button
              variant="ghost"
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide max-w-2xl mx-auto">
          {categories.map((cat) => (
            <button
              ...
              className={cn(
                "shrink-0 rounded-full px-4 py-2.5 text-sm font-semibold transition-all duration-300 active:scale-95",
      ...
      <div className="flex-1 overflow-y-auto px-5 space-y-5 pt-4">
            ...
            <div className="space-y-3">
              {txns.map((txn, idx) => {
                ...
                return (
                  <div
                    key={txn.id}
                    className="rounded-[1.5rem] bg-card p-4 shadow-sm border border-border/50 card-hover"
                      ...
                      <div className="text-right">
                        ...
                        <div className="flex gap-1">
                          {onEmotionTag && (
                            ...
                            >
                              <Smile className="mr-1.5 h-4 w-4" strokeWidth={2} />
                              Feel
                            </Button>
                          )}
                          <Button
                          ...
                          >
                            <Tag className="mr-1.5 h-4 w-4" strokeWidth={2} />
                            Context
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 border-t border-border/50 pt-4 animate-fade-in">
                        {contextTags.map((tag) => (
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            className="h-10 rounded-full border-2 border-border hover:bg-primary/10 hover:text-primary hover:border-primary/30 font-semibold"
                            onClick={() => {
        ...
        {filteredTransactions.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
            <div className="flex h-24 w-24 items-center justify-center rounded-[2rem] bg-secondary">
