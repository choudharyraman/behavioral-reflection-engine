      ...
      <div className="rounded-[1.75rem] bg-card p-5 shadow-sm border border-border/50">
        ...
        <div className="flex mb-3">
          ...
          <div className="flex flex-1 justify-between">
            {hourBlocks.map(block => (
              <span key={block.label} className="text-[10px] font-semibold text-muted-foreground w-11 text-center">
                {block.label}
              </span>
            ))}
          </div>
        ...
        <div className="space-y-2">
          {dayKeys.map((day, dayIndex) => (
            <div key={day} className="flex items-center gap-2">
              <span className="w-9 text-[11px] font-semibold text-muted-foreground">
                {days[dayIndex]}
              </span>
                ...
                {hourBlocks.map((block) => {
                  ...
                  return (
                    <div
                      key={`${day}-${block.label}`}
                      className={cn(
                        "h-8 flex-1 rounded-lg transition-all duration-300 hover:scale-105 cursor-pointer",
        ...
        <div className="mt-5 flex items-center justify-center gap-3">
          ...
          <div className="flex gap-1">
            {['bg-secondary/50', 'bg-primary/15', 'bg-primary/30', 'bg-primary/50', 'bg-primary/70', 'bg-primary'].map((cls, i) => (
