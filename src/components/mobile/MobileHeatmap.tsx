        ...
        <div className="flex mb-2">
          ...
          <div className="flex flex-1 justify-between">
            {hourBlocks.map(block => (
              <span key={block.label} className="text-[10px] text-muted-foreground w-10 text-center">
                {block.label}
              </span>
            ))}
          </div>
        ...
        <div className="space-y-1.5">
          {dayKeys.map((day, dayIndex) => (
            <div key={day} className="flex items-center gap-2">
              <span className="w-4 text-[10px] font-medium text-muted-foreground">
                {days[dayIndex]}
              </span>
        ...
        <div className="mt-4 flex items-center justify-center gap-2">
          ...
          <div className="flex gap-0.5">
            {['bg-muted/30', 'bg-primary/20', 'bg-primary/40', 'bg-primary/60', 'bg-primary/80', 'bg-primary'].map((cls, i) => (
              <div key={i} className={cn("h-3 w-3 rounded", cls)} />
            ))}
          </div>
